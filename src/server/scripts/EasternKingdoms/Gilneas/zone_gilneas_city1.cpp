...
enum eZoneGilneas
{
    ...
    MOVE_TO_PRINCE_LIAM                          = 2,
    MOVE_TO_DOOR                                 = 3,
    MOVE_TO_HOMEPOSITION                         = 4,
    MOVE_TO_PLAYER                               = 5,
    MOVE_TO_MARKER                               = 6,
    MOVE_TO_END                                  = 7,

    ACTION_START_ANIM_MERCANT                    = 1,
    ACTION_START_ANIM_LIAM                       = 2,
    ACTION_START_ANIM_CITIZEN                    = 3,

    PANICKED_CITIZEN_TEXT_00                     = 0,
    PRINCE_LIAM_GREYMANE_TEXT_00                 = 0,
    PRINCE_LIAM_GREYMANE_TEXT_01                 = 1,
    PRINCE_LIAM_GREYMANE_TEXT_02                 = 2,
    WORGEN_ENRAGE_TEXT_EMOTE                     = 0,
    LIAM_ATTACK_WORGEN_TEXT                      = 0,
    FRIGHTENED_CITIZEN_01_FLEE_TEXT              = 0,
    FRIGHTENED_CITIZEN_02_FLEE_TEXT              = 0
};
...
struct npc_gilneas_city_guard_gate_34864 : public ScriptedAI
{
    npc_gilneas_city_guard_gate_34864(Creature* creature) : ScriptedAI(creature) { }
    ...
    bool OnQuestAccept(Player* /*player*/, Creature* creature, Quest const* quest) override
    {
        if (quest->GetQuestId() == QUEST_LOCKDOWN)
            if (Creature* citizen = creature->FindNearestCreature(NPC_PANICKED_CITIZEN, 20.0f))
                citizen->AI()->Talk(PANICKED_CITIZEN_TEXT_00);
        return true;
    }
                ...
                switch (eventId)
                {
                    ...
                    case EVENT_START_TALK_TO_GUARD:
                    {
                        if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                            Talk(PRINCE_LIAM_GREYMANE_TEXT_00, player);
                    ...
                    {
                        if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                            Talk(PRINCE_LIAM_GREYMANE_TEXT_01, player);
                    ...
                    {
                        if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                            Talk(PRINCE_LIAM_GREYMANE_TEXT_02, player);
...
struct npc_rampaging_worgen_35660 : public ScriptedAI
{
    npc_rampaging_worgen_35660(Creature* creature) : ScriptedAI(creature) { }
            ...
            switch (eventId)
            {
                ...
                case EVENT_ATTACK_LIAM:
                {
                    if (Creature* liam = ObjectAccessor::GetCreature(*me, m_liamGUID))
                    {
                        ...
                        me->Attack(liam, true);
                        liam->Attack(me, true);
                        liam->AI()->Talk(LIAM_ATTACK_WORGEN_TEXT);
                    }
                    break;
                }
...
struct npc_prince_liam_greymane_34913 : public ScriptedAI
{
    npc_prince_liam_greymane_34913(Creature* c) : ScriptedAI(c)
    {
        m_events.ScheduleEvent(EVENT_START_NEXT_SHOWFIGHT, 40s);
    }
    ...
    void JustSummoned(Creature* summon) override
    {
        if (summon->GetEntry() == NPC_RAMPAGING_WORGEN_35660)
        {
            ...
            summon->GetAI()->DoAction(ACTION_START_ANIM_LIAM);
        }
    }
...
struct npc_gilneas_city_guard_34916 : public ScriptedAI
{
    npc_gilneas_city_guard_34916(Creature* creature) : ScriptedAI(creature)
    {
        Initialize();
    }
...
struct npc_rampaging_worgen_34884 : public ScriptedAI
{
    npc_rampaging_worgen_34884(Creature* creature) : ScriptedAI(creature) { }
    ...
    void MovementInform(uint32 type, uint32 point) override
    {
        if (type == POINT_MOTION_TYPE)
        {
            switch (point)
            {
                ...
                case MOVE_TO_PLAYER:
                {
                    Talk(FRIGHTENED_CITIZEN_01_FLEE_TEXT);
...
struct npc_frightened_citizen_35836 : public ScriptedAI
{
    npc_frightened_citizen_35836(Creature* creature) : ScriptedAI(creature) { }
    ...
    void MovementInform(uint32 type, uint32 point) override
    {
        if (type == POINT_MOTION_TYPE)
        {
            switch (point)
            {
                ...
                case MOVE_TO_PLAYER:
                {
                    Talk(FRIGHTENED_CITIZEN_02_FLEE_TEXT);
...
/// alexkulya: ToDo knocking sound & animation (spell script?).
...
public:
    ...
    {
        npc_gilnean_city_guard_35504AI(Creature* creature) : ScriptedAI(creature)
        {
            Initialize();
        }
...
public:
    ...
    {
        npc_lord_darius_crowley_35552AI(Creature* creature) : ScriptedAI(creature)
        {
            Initialize();
        }
            ...
            if (!m_AnimRunning)
                if (Player* player = who->ToPlayer())
                    ...
                    if (!HasPlayer(player->GetGUID()))
                    {
                        m_AnimRunning = true;
                        m_playerGUID = player->GetGUID();
                        m_events.RescheduleEvent(EVENT_START_TALK, 1s);
                    }
        }
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_START_TALK:
                    {
                        ...
                        if (ObjectAccessor::GetPlayer(*me, m_playerGUID))
                            if (ObjectAccessor::GetCreature(*me, m_kingGUID))
                                ...
                                {
                                    m_events.ScheduleEvent(EVENT_TALK_PART_00, 1s);
                                    break;
                                }
                    ...
                    case EVENT_TALK_PART_00:
                    {
                        ...
                        m_events.ScheduleEvent(EVENT_TALK_PART_01, 9s);
                        break;
                    }
                    case EVENT_TALK_PART_01:
                    {
                        if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                            Talk(0, player);
                        m_events.ScheduleEvent(EVENT_TALK_PART_02, 9s);
                        break;
                    }
                    case EVENT_TALK_PART_02:
                    {
                        ...
                        m_events.ScheduleEvent(EVENT_TALK_PART_03, 6s);
                        break;
                    }
                    case EVENT_TALK_PART_03:
                    {
                        if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                            Talk(1, player);
                        m_events.ScheduleEvent(EVENT_TALK_PART_04, 9s);
                        break;
                    }
                    case EVENT_TALK_PART_04:
                    {
                        if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                            Talk(2, player);
                        m_events.ScheduleEvent(EVENT_TALK_PART_05, 8s);
                        break;
                    }
                    case EVENT_TALK_PART_05:
                    {
                        ...
                        m_events.RescheduleEvent(EVENT_TALK_PART_06, 30s, 45s);
                        break;
                    }
        ...
        void MovementInform(uint32 type, uint32 id) override
        {
                ...
                switch (id)
                {
                        ...
                        if (m_movePart == 3)
                            if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
                                if (Creature* darius = ObjectAccessor::GetCreature(*me, m_dariusGUID))
                                {
                                    ...
                                    m_events.ScheduleEvent(EVENT_TALK_PERIODIC, 6s);
                                }
                        break;
                    }
                    case 14:
                    {
                        if (m_movePart == 5) // endpoint run 2
                            m_events.ScheduleEvent(EVENT_MOVE_PART5, 10ms);
                        break;
                    }
                    case 16:
                    {
                        if (m_movePart == 3) // endpoint run 1
                            m_events.ScheduleEvent(EVENT_MOVE_PART3, 10ms);
                        break;
                    }
                }
            else if (type == POINT_MOTION_TYPE)
            {
                switch (id)
                {
                    ...
                    {
                        m_events.ScheduleEvent(EVENT_MOVE_PART1, 10ms);
                        break;
                    }
                }
            }
            else if (type == EFFECT_MOTION_TYPE)
            {
                switch (m_movePart)
                {
                    ...
                    {
                        m_events.ScheduleEvent(EVENT_MOVE_PART2, 10ms);
                        break;
                    }
                    case 4: // jump over bridge
                    {
                        m_events.ScheduleEvent(EVENT_MOVE_PART4, 10ms);
                        break;
                    }
                }
            }
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                switch (eventId)
                {
                    ...
                    case EVENT_TALK_PERIODIC:
                    {
                        ...
                        if (m_movePart < 5)
                            m_events.ScheduleEvent(EVENT_TALK_PERIODIC, 5s, 8s);
                        break;
                    }
                }
            }
    ...
    struct npc_bloodfang_stalker_35229AI : public ScriptedAI
    {
                    ...
                    if (horse->GetEntry() == NPC_CROWLEYS_HORSE)
                        if (horse->GetDistance(me) < 15.0f)
                            if (urand(0, 100) < 30)
                            {
                                ...
                                m_isFollowing = true;
                                m_events.ScheduleEvent(EVENT_GIVE_UP, 4s, 10s);
                            }
        }
        ...
        void DamageTaken(Unit* /*who*/, uint32& /*damage*/) override
        {
            if (!m_enrage && me->GetHealthPct() < 90.0f)
            {
                me->CastSpell(me, SPELL_ENRAGE_8599);
                m_enrage = true;
                m_events.ScheduleEvent(EVENT_ENRAGE_COOLDOWN, 60s);
            }
            if (!m_isTalking)
                if (urand(0, 100) < 30)
                {
...
public:
    ...
    struct npc_sister_almyra_44468AI : public ScriptedAI
    {
        ...
        {
            m_events.Reset();
            m_events.ScheduleEvent(EVENT_CAST_RENEW, 1s);
        }
            ...
            while (uint32 eventId = m_events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_CAST_RENEW:
                    {
                        ...
                        m_events.ScheduleEvent(EVENT_CAST_RENEW, 5s);
                        break;
                    }
                }
            }
...
public:
    ...
    struct npc_rebel_cannon_35317AI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            m_events.ScheduleEvent(EVENT_CHECK_STALKER, 1s);
        }
        ...
        void DoAction(int32 param) override
        {
            switch (param)
            {
                case EVENT_START_MOVIE:
                {
                    ...
                    m_events.RescheduleEvent(EVENT_START_MOVIE, 2s);
                    break;
                }
            }
        }
...
public:
    ...
    {
        npc_northgate_rebel_41015AI(Creature* creature) : ScriptedAI(creature)
        {
            Initialize();
        }
...
void AddSC_zone_gilneas_city1()
{
    ...
    RegisterCreatureAI(npc_gilneas_city_guard_gate_34864);
    new npc_prince_liam_greymane_34850();
    RegisterCreatureAI(npc_rampaging_worgen_35660);
    RegisterCreatureAI(npc_prince_liam_greymane_34913);
    RegisterCreatureAI(npc_gilneas_city_guard_34916);
    RegisterCreatureAI(npc_rampaging_worgen_34884);
    RegisterCreatureAI(npc_frightened_citizen_34981);
    RegisterCreatureAI(npc_frightened_citizen_35836);
