            ...
            if (Unit* caster = GetCaster())
            {
                if (caster->HasAura(144966))
                {
                    if (roll_chance_i(5))
                    {
                        ...
                        {
                            if (caster->GetSpellHistory()->HasCooldown(SPELL_SCHAMAN))
                                caster->GetSpellHistory()->ResetCooldown(SPELL_SCHAMAN, true);
                        }
                    }
                }
            }
         ...
         void HandleAfterCast()
        {
            if (Player* _player = GetCaster()->ToPlayer())
            {
                if (_player->HasAura(144637))
                    _player->GetSpellHistory()->ReduceChargeCooldown(sSpellCategoryStore.LookupEntry(sSpellMgr->GetSpellInfo(3045)->ChargeCategoryId), 1 * IN_MILLISECONDS);
