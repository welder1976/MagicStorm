			...
			void EnterCombat(Unit* /*attacker*/) override
			{
				...
				switch (me->GetEntry())
				{
					...
					case NPC_CAPTAIN_YATHAE_MOONSTRIKE:
                         ...
                         events.ScheduleEvent(EVENT_INCORPOREAL_TELEPORT, Seconds(urand(30, 38)), 0, ((THE_HUNTRESS), (THE_PRIESSTES));
				...
				switch (action)
				{
					case ACTION_EVADE:
					{
                      switch (me->GetEntry())
                      {
                          case NPC_HUNTRESS_KASPARIAN:
                          {
                              ...
                              me->RemoveAuraDueToSpell(SPELL_GHOST_AURA);
                              break;
                          }
                          case NPC_CAPTAIN_YATHAE_MOONSTRIKE:
                          case NPC_PRIESTESS_LUNASPYRE:
					...
					case ACTION_PHASE_2:
					{
						...
						switch (me->GetEntry())
						{
							...
							{
								me->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE|UNIT_FLAG_NOT_SELECTABLE);
								events.ScheduleEvent(EVENT_TELEPORT_CENTER_COMBAT);
								//Enable Movement
								break;
							}
							case NPC_PRIESTESS_LUNASPYRE:
					...
					case ACTION_PHASE_3:
					{
						...
						switch (me->GetEntry()
						{
							case NPC_CAPTAIN_YATHAE_MOONSTRIKE:
							{
							     me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE|UNIT_FLAG_NOT_SELECTABLE);
								 me->AddAura(SPELL_GHOST_AURA, me);
								 events.ScheduleEvent(EVENT_BACK_TO_GHOST);
								 //Disable Movement
								 break;
							}
							case NPC_PRIESTESS_LUNASPYRE:
